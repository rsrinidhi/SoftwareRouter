...
struct osfpv2_neighbor
{
    uint32_t neighbor_rid;
    uint8_t helloint;
    struct osfpv2_neighbor* next;
    
}__attribute__ ((packed));

struct ospfv2_hello_packet
{
...
struct lsa_packet
{
    ...
    uint32_t subnet;
    uint32_t mask;
    uint32_t neighbour_rid;
    struct timeval time;
    struct lsa_packet* next;
...
{
    struct lsa_packet* topology_entry;
    uint8_t cost;
